// scripts/components/KimetsuNoVaibaSection.js

const { useState, useEffect } = React;

// DEFINICIÓN COMPLETA DEL PRODUCT CARD (incluyendo Dark Mode)
...
function KimetsuNoVaibaSection({ addToCart }) {
    const [products, setProducts] = useState([]);
    
    useEffect(() => {
        // CORRECCIÓN: Carga los datos del archivo JSON
        fetch('./data/kimetsunoyaiba.json')
            .then(response => response.json())
            .then(data => setProducts(data))
            .catch(error => console.error('Error loading Kimetsu No Yaiba:', error));
    }, []);
    ...
    return (
        ...
        <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white px-5 md:px-16 py-10">
            <h2 className="text-3xl font-normal mb-8 uppercase dark:text-white">KIMETSU NO YAIBA</h2>
...
window.KimetsuNoVaibaSection = KimetsuNoVaibaSection;