// scripts/components/FiguraAccionSection.js
...
function FiguraAccionSection({ addToCart }) {
    ...
    useEffect(() => {
        fetch('./data/figuras.json')
            ...
            .catch(error => console.error('Error loading Figuras:', error));
    }, []);

    return (
        <div className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white px-5 md:px-16 py-10">
            <h2 className="text-3xl font-normal mb-8 uppercase dark:text-white">FIGURA DE ACCIÓN</h2>
...
window.FiguraAccionSection = FiguraAccionSection;